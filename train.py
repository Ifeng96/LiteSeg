...
ap.add_argument('--backbone_network', required=True,
                help = 'name of backbone network [shufflenet, mobilenet, and darknet] ',default='darknet')
args = ap.parse_args()
...
#########################
# 训练保存名称，通过NAME进行区分
NAME = 'LiteSeg_' + args.backbone_network
...
#   训练流程
#########################
# pretrained_weight = f'pretrained_models/liteseg-{args.backbone_network}-cityscapes.pth'
liteseg = LiteSeg(args.backbone_network, class_num, None, is_train=False)
# 训练流程搭建，具体见utils.framework.MyFrame类
solver = MyFrame(class_num, liteseg, metric, learning_rate)
# 模型参数量打印及记录
